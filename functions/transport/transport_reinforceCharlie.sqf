...
for '_i' from 1 to _reinforcementCount do {
	...
	_unit = backup_charlie createUnit [_soldier, _position, [],0, "NONE"];
...
private _allAboard = !(({_x in _transport} count units backup_charlie) == _reinforcementCount || ( count units backup_charlie >= _reinforcementCount));
if (_allAboard) exitWith {
		call compile format['[%1] execVM "functions\transport\transport_reinforceCharlie.sqf"', _transport];
};

waitUntil {{_x in _transport} count units backup_charlie == _reinforcementCount};
