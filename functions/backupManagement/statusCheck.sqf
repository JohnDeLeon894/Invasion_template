...
private _reportStatus = {
	if (isNil('_orders')) then {
		_orders = _group getVariable['currentOrders', 'status'];
	};

	// private _markerText = formatText['Status:', lineBreak, 'Squad Strength: ', _aliveCount, lineBreak, 'Current Orders:', _orders];

	private _markerText =  format['Unit Strength: %1 / Current Orders %2', _aliveCount, _orders];
	...
	private _statusString = format['This is %1, we have %2 units. We are at coordinates %3 We are executing a %4 order.', _groupLeader, _aliveCount, _mapPos, _orders];
